...
void nm_create_windows(void)
{
    ...
    action_size = NM_SET_POS(screen_y - 1, action_cols, screen_x - action_cols,
                             1);
                      ...
                      int status)
{
    ...
    if (nm_str_cmp_st(nm_vect_str(&vm->main, NM_SQL_USBF), "1") == NM_OK) {
        nm_str_format(&buf, "%-12s%s [%s]", "usb: ", "enabled",
                      (nm_str_cmp_st(nm_vect_str(&vm->main, NM_SQL_USBT),
        ...
        for (;;) {
            ...
            if (ch == NM_KEY_ENTER) {
                ...
                for (size_t y = 3, l = 0; l < rows && n < hotkey_num;
                     n++, y++, l++)
                    mvwprintw(action_window, y, 2, "%-10s%s", keys[n],
                              values[n]);

                last = n;
                if (last < hotkey_num) {
                    perc = 100 * last / hotkey_num;
                    nm_str_format(&help_title, _(
                                      "nEMU %s - help [%d%%]"), NM_VERSION,
                                  perc);
                } else {
                    nm_str_format(&help_title, _(
                                      "nEMU %s - help [end]"), NM_VERSION);
                }
