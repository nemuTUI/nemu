...
void nm_cfg_init(void)
{
    ...
    if (nm_get_opt_param(ini, NM_INI_S_MAIN, NM_INI_P_HL, &tmp_buf) == NM_OK) {
        if (tmp_buf.len != 6)
            nm_bug(_(
                       "cfg: incorrect color value %s, example:5fafff"),
                   tmp_buf.data);
    ...
    nm_str_trunc(&tmp_buf, 0);
    if (nm_get_opt_param(ini, NM_INI_S_DMON, NM_INI_P_DTMT,
                         &tmp_buf) == NM_OK) {
...
static void nm_generate_cfg(const char *home, const nm_str_t *cfg_path)
{
    ...
    printf(NM_STRING(
               NM_USR_PREFIX) "/share/nemu/templates/config/nemu.cfg.sample\n");
...
#ifdef NM_WITH_VNC_CLIENT
        nm_get_input(_(
                         "VNC client path (enter \"/bin/false\" if you connect other way)"),
                     &vnc_bin);
        nm_get_input(_("VNC client arguments"), &vnc_args);
#endif
#ifdef NM_WITH_SPICE
        nm_get_input(_(
                         "SPICE client path(enter \"/bin/false\" if you connect other way)"),
                     &spice_bin);
